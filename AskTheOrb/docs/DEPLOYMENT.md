# Deploying Ask the Orb to the App Store
...
|---|---|---|
| Bundle ID | `com.asktheorb.app` | `project.yml`, `fastlane/Appfile`, App Store Connect |
| Monthly product | `com.asktheorb.pro.monthly` | `App/Store/StoreConfig.swift`, `Configuration/Products.storekit`, App Store Connect |
| Yearly product | `com.asktheorb.pro.annual` | same three |
| Lifetime product | `com.asktheorb.pro.lifetime` | same three |

Use a domain you control, in reverse — `com.yourcompany.asktheorb`. Bundle IDs
are permanent once registered.

```bash
# From AskTheOrb/, replace the placeholder prefix everywhere at once:
grep -rl 'com.asktheorb' . --exclude-dir=.git \
  | xargs sed -i '' 's/com\.asktheorb/com.yourcompany.asktheorb/g'
```
...
2. [appstoreconnect.apple.com/apps](https://appstoreconnect.apple.com/apps)
   → **+** → New App.
   - Platform: iOS
   - Name: `Ask the Orb` (must be globally unique — have a backup ready)
   - Primary language: English (U.S.)
   - Bundle ID: the one you just registered
   - SKU: anything internal, e.g. `ASKTHEORB001`
...
**Subscriptions** → create a subscription group named `Orb Pro`, then
add two subscriptions inside it:

| Reference name | Product ID | Duration | Price |
|---|---|---|---|
| Pro Monthly | `com.asktheorb.pro.monthly` | 1 month | $2.99 |
| Pro Yearly | `com.asktheorb.pro.annual` | 1 year | $14.99 |
...
|---|---|---|
| Pro Lifetime | `com.asktheorb.pro.lifetime` | $29.99 |
...
- **Screenshots.** Required: 6.7" (1290×2796) and 6.5" (1242×2688). Generate
  ...
  simulator with ⌘S.
- **Age rating.** Answer the questionnaire. Ask the Orb rates 4+ —
